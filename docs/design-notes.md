...
- **Metricas durables fuera del repo**: `~/.claude/slice-runner/metrics.jsonl` (append-only, no versionado, sobrevive al descarte) para medir "cuando subir de nivel". Lo escribe y lo agrega el programa el mismo (`LocalMetricsLog`, el subcomando `metrics`).
