...
| `skills/slice-runner/scripts/discover_controles.py` / `discover_conventions.py` / `discover_prior_art.py` | Helpers de descubrimiento | Los usa `slice-spec` para **proponer** los controles y las fuentes de convencion del repo. El tercero busca **trabajo previo y piezas reutilizables** antes de trocear, para que una slice no construya lo que ya existe ni traduzca lo que otra esta jubilando. Descubren y no deciden: confirma la persona. |
