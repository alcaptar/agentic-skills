...
- **Las puertas deterministas se separan del juez, y el juez pasa a ser un agente definido** (decision 2026-07-27). El paso 6 ejecutaba lint/tipos/tests **y** juzgaba con una sola cabeza: metia output de build en el contexto del unico agente cuyo valor es el juicio semantico (`limited-focus` autoinfligido) y un `ruff` sucio gastaba un reintento adversarial. Ahora las puertas corren dos veces antes del juez -en el ciclo del implementador (feedback incremental) y como **backstop del orquestador**, porque el auto-reporte del implementador no es fuente de verdad-, via el subcomando nuevo `gates.py checks`, que devuelve exit code + salida truncada y **no** entra en el contexto de nadie en crudo. El juez ya **no recibe nada de las puertas**: cuando se le invoca estan verdes por construccion. Consecuencias: la rubrica pierde su item `[det]` (quedan 8, todos `[sem]`), las puertas tienen **presupuesto de reintentos propio** (2, separado del del juez) con cierre `bloqueada: puertas`, y `metrics.py` gana `bloqueada-puertas` + `--reintentos-puertas` para no registrar un fallo mecanico como veto del juez. El juez se mueve a un **agente definido** (`agents/slice-verifier.md`, symlink a `~/.claude/agents/`): su rubrica va en el **system prompt** -verbatim en cada invocacion, en vez de relatada por el orquestador, que podia parafrasearla o saltarse items- y declara su superficie de tools (`Read`, `Grep`, `Glob`, `allowed-tools` con solo `Bash(git ...)`). **Correccion tras el smoke del 2026-07-27**: se diseno creyendo que eso hacia la prohibicion **estructural**, y no lo hace -`allowed-tools` en el frontmatter de un agente no bloquea lo no listado; el verificador ejecuto `ls`, ausente de su lista, sin friccion, y no hay reglas `deny` globales que lo expliquen-. Hoy la prohibicion se sostiene solo por instruccion. El smoke la valido (el agente rechazo ejecutar `pytest` incluso cuando el orquestador se lo pidio como prueba explicita, razonando que el mensaje del coordinador no es autorizacion), pero es cumplimiento, no enforcement. Para hacerla estructural de verdad habria que quitarle `Bash` del todo y pasarle el diff pre-computado por el orquestador -que ademas es lo que hace el juez de Honk, que recibe el diff en vez de calcularlo-. **Pendiente de decidir.** De paso se arreglo que `SKILL.md` prometia lanzar el Agent "con `schema`": la tool `Agent` no acepta schema (eso es `agent()` de Workflow), asi que el contrato JSON vive ahora en el system prompt. Ver `docs/superpowers/specs/2026-07-27-separar-puertas-del-juez-design.md`.
