...
_ROLE_ADAPTERS = (
    "src/slice_runner/infrastructure/claude_verifier.py",
    "src/slice_runner/infrastructure/claude_implementer.py",
    "src/slice_runner/infrastructure/claude_understanding.py",
)

_TELEMETRY_WRITES = frozenset({("trace", "record"), ("spend_log", "record"), ("tool_uses", "record_after")})
"""The three calls `docs/conventions/architecture.md` reserves for whoever runs the harness call.

Written once in `HarnessInvocationRunner.call`, they are the register the common piece exists so no
role adapter has to reimplement it. A role that reaches for one of these again -- through the
runner or by accepting `HarnessTelemetry` a second time -- is the reopened repartition #257/#259
already paid for once, so the scan does not trust the constructor shape alone to keep it shut.
"""


def _telemetry_write_calls(source: str) -> list[int]:
    """Lines that call `.trace.record(`, `.spend_log.record(` or `.tool_uses.record_after(`."""
    tree = ast.parse(source)
    ...
    return sorted(
        ...
        and isinstance(node.func, ast.Attribute)
        and isinstance(node.func.value, ast.Attribute)
        and (node.func.value.attr, node.func.attr) in _TELEMETRY_WRITES
    )


def test_no_role_adapter_writes_telemetry_of_its_own_call() -> None:
    """Criterion 2 of #361: a role adapter that reimplements the register slips past the constructor

    shape check silently, because nothing about `self._calls.call(...)` stops it from also holding a
    `HarnessTelemetry` on the side. This measures the tree instead of trusting that shape to hold.
    """
    offenders = {path: lines for path in _ROLE_ADAPTERS if (lines := _telemetry_write_calls(_read(_ROOT / path)))}

    assert not offenders, (
        "these role adapters write telemetry on their own instead of through the runner:\n"
        + "\n".join(
            f"  {path}: line(s) {', '.join(str(line) for line in lines)}" for path, lines in sorted(offenders.items())
        )
    )


def test_the_scan_catches_the_three_telemetry_writes_by_their_qualified_name() -> None:
    """A scan that only recognises one of the three spellings would miss the other two coming back."""
    source = "\n".join(
        [
            "class RoleAdapter:",
            "    def act(self):",
            "        self._telemetry.trace.record(x)",
            "        self._telemetry.spend_log.record(y)",
            "        self._telemetry.tool_uses.record_after(z)",
            "        self._reader.read(w)",
        ]
    )

    assert _telemetry_write_calls(source) == [3, 4, 5]
