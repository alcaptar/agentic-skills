...
- **Las puertas deterministas se separan del juez, y el juez pasa a ser un agente definido** (decision 2026-07-27). El paso 6 ejecutaba lint/tipos/tests **y** juzgaba con una sola cabeza: metia output de build en el contexto del unico agente cuyo valor es el juicio semantico (`limited-focus` autoinfligido) y un `ruff` sucio gastaba un reintento adversarial. Ahora las puertas corren dos veces antes del juez -en el ciclo del implementador (feedback incremental) y como **backstop del orquestador**, porque el auto-reporte del implementador no es fuente de verdad-, via el subcomando nuevo `gates.py checks`, que devuelve exit code + salida truncada y **no** entra en el contexto de nadie en crudo. El juez ya **no recibe nada de las puertas**: cuando se le invoca estan verdes por construccion. Consecuencias: la rubrica pierde su item `[det]` (quedan 8, todos `[sem]`), las puertas tienen **presupuesto de reintentos propio** (2, separado del del juez) con cierre `bloqueada: puertas`, y `metrics.py` gana `bloqueada-puertas` + `--reintentos-puertas` para no registrar un fallo mecanico como veto del juez. El juez se mueve a un **agente definido** (`agents/slice-verifier.md`, symlink a `~/.claude/agents/`): su rubrica va en el **system prompt** -verbatim en cada invocacion, en vez de relatada por el orquestador, que podia parafrasearla o saltarse items- y su `allowed-tools` (`Read`, `Grep`, `Glob`, solo `Bash(git ...)`) hace **estructural** la prohibicion de re-ejecutar puertas, en vez de un ruego que `selective-hearing` puede ignorar. De paso se arreglo que `SKILL.md` prometia lanzar el Agent "con `schema`": la tool `Agent` no acepta schema (eso es `agent()` de Workflow), asi que el contrato JSON vive ahora en el system prompt. Ver `docs/superpowers/specs/2026-07-27-separar-puertas-del-juez-design.md`.
