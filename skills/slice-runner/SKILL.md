...
- **Los subagentes no son un detalle de implementacion: son la garantia.** Esta skill **no puede ejecutarse sin ellos**, y por eso **invocarla cuenta como pedirlos**: no es iniciativa del agente, es la skill haciendo su trabajo. Si el entorno los veta (veto global al Agent tool, politica de la organizacion, `slice-verifier` sin resolver), **para en el paso 3**. El por que no se decide aqui: sale de aplicar el criterio general de degradacion (`~/.claude/CLAUDE.md`, seccion Subagentes) a **este** artefacto. Y el artefacto de esta skill es una **PR con veredicto PASA**, donde el veredicto *es* la afirmacion de que se verifico: degradado seria falso, y **falso de forma invisible aguas abajo** -quien revise la PR asume que paso el pipeline-. No hay forma de declarar esa degradacion dentro del artefacto, asi que se cae del lado de parar. Parar no cuesta nada irreversible: no hay nada en produccion. Fail-closed, igual que `pr-hygiene` y `diff-bundle`. (`deploy-watch` cae del otro lado y degrada: **mismo criterio, artefacto distinto**, no incoherencia.)
