...
- **Si el entorno veta los subagentes, degrada avisando; no pares.** Invocar esta skill cuenta como pedir el colector y el `sre`, pero si una restriccion los impide (veto global al Agent tool, politica de la organizacion), **sigue adelante**: recoge las senales tu mismo y haz el RCA inline. Al contrario que en `slice-runner` -que **para**, porque ahi el subagente ES la garantia y una verificacion inline no verifica nada-, aqui lo que se pierde es higiene de contexto, no validez: el cambio ya esta en produccion, `deploy_core.py` sigue decidiendo con la misma logica determinista y el veredicto se sostiene igual. Quedarte sin monitorizar un deploy ya mergeado es peor que monitorizarlo con el contexto sucio. Obligatorio en ese caso: **decirlo al arrancar** y **repetirlo en el informe final**, advirtiendo de que en una ventana larga el contexto se acumula tick a tick y el watch pierde fiabilidad -si la ventana es larga, propon acortarla o vigilar menos senales-. Lo que **no** se degrada nunca: `deploy_core.py` decide siempre, jamas la impresion del agente.
